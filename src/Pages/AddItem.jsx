...
export default function InputForm({ onAddItem, onClose, initialData }) {
  const form = useForm({
  ...
  // eslint-disable-next-line react/prop-types
  const [totalPrice, setTotalPrice] = useState(initialData ? initialData.price : 0);

  // Pricing logic
  const prices = {
    BBQ: {
      base: 100,
      types: { Pahadi: 20, Tandoori: 25, Italian: 30 },
      cheese: { "with Cheese": 15, "without Cheese": 0 },
    },
    Maggie: {
      base: 50,
      types: { Plain: 10, Veg: 15, Italian: 20 },
      cheese: { "with Cheese": 10, "without Cheese": 0 },
    },
    "Garlic Bread": {
      base: 80,
      types: { Cheese: 10, Sweetcorn: 15, "Onion & Capsicum": 20 },
      cheese: { "with Cheese": 0, "without Cheese": 0 },
    },
  };
